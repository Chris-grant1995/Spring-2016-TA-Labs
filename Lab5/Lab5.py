...
def sales_report():
    ...
    while True:
        try:
            dict = pickle.load(f)
            print(sale_report_line.format(dict['sku'], dict['quantity'], dict['total']))
        except EOFError:
            break
    f.close()
...
def transaction():
    ...
    try:
        ...
        max = data["quantity"]
        price = data['price']
        amt = int(input("How many " + data['name'] + " would you like? :"))
        if amt <= max:
            sale = {}
            sale['sku'] = sku
            sale['quantity'] = amt
            sale['total'] = amt*price
...
def inventory_management():
    ...
    for k in keys:
        print(inventory_report_line.format(k,inventory[k]['name'],inventory[k]["quantity"],inventory[k]["price"] ))
        #print(k)
    ...
    sku = input("Enter a SKU")
    if sku in keys:
        print(inventory[sku]['name'])
        amt = int(input("Enter the new amount:"))
        price = float(input("Enter the new price"))
        inventory[sku]["quantity"] = amt
